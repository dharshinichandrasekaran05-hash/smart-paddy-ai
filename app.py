...
# Compatibility helpers: keep app startup independent of optional newer
# i18n additions so an older deployed utils/i18n.py cannot break imports.
try:
    from utils.i18n import translate_treatment_recovery
except ImportError:
    def translate_treatment_recovery(value: str, lang: str) -> str:
        return value

_TREATMENT_HEADERS = {
    "english": ["Treatment Option", "Priority", "Success Rate", "Recovery Time (days)", "Cost", "Eco-Friendly", "Best Applied At Stage"],
    "tamil": ["சிகிச்சை விருப்பம்", "முன்னுரிமை", "வெற்றி விகிதம்", "மீட்பு காலம் (நாட்கள்)", "செலவு", "சுற்றுச்சூழல் நட்பு", "சிறந்த பயன்பாட்டு நிலை"],
    "telugu": ["చికిత్స ఎంపిక", "ప్రాధాన్యత", "విజయ రేటు", "కోలుకునే సమయం (రోజులు)", "ఖర్చు", "పర్యావరణ అనుకూలం", "ఉత్తమంగా వర్తించే దశ"],
    "kannada": ["ಚಿಕಿತ್ಸೆ ಆಯ್ಕೆ", "ಆದ್ಯತೆ", "ಯಶಸ್ಸಿನ ಪ್ರಮಾಣ", "ಚೇತರಿಕೆಯ ಅವಧಿ (ದಿನಗಳು)", "ವೆಚ್ಚ", "ಪರಿಸರ ಸ್ನೇಹಿ", "ಅತ್ಯುತ್ತಮ ಅನ್ವಯ ಹಂತ"],
    "malayalam": ["ചികിത്സാ ഓപ്ഷൻ", "മുൻഗണന", "വിജയ നിരക്ക്", "വീണ്ടെടുക്കൽ സമയം (ദിവസങ്ങൾ)", "ചെലവ്", "പരിസ്ഥിതി സൗഹൃദം", "ഏറ്റവും അനുയോജ്യമായ ഘട്ടം"],
    "hindi": ["उपचार विकल्प", "प्राथमिकता", "सफलता दर", "ठीक होने का समय (दिन)", "लागत", "पर्यावरण अनुकूल", "उपयोग का सर्वोत्तम चरण"],
    "bengali": ["চিকিৎসার বিকল্প", "অগ্রাধিকার", "সাফল্যের হার", "সুস্থতার সময় (দিন)", "খরচ", "পরিবেশবান্ধব", "প্রয়োগের সেরা পর্যায়"],
    "marathi": ["उपचार पर्याय", "प्राधान्य", "यशाचे प्रमाण", "बरे होण्याचा कालावधी (दिवस)", "खर्च", "पर्यावरणपूरक", "लागू करण्याची सर्वोत्तम अवस्था"],
    "gujarati": ["સારવાર વિકલ્પ", "પ્રાથમિકતા", "સફળતા દર", "સુધારાનો સમય (દિવસો)", "ખર્ચ", "પર્યાવરણને અનુકૂળ", "લાગુ કરવાનો શ્રેષ્ઠ તબક્કો"],
    "punjabi": ["ਇਲਾਜ ਵਿਕਲਪ", "ਤਰਜੀਹ", "ਸਫਲਤਾ ਦਰ", "ਠੀਕ ਹੋਣ ਦਾ ਸਮਾਂ (ਦਿਨ)", "ਲਾਗਤ", "ਵਾਤਾਵਰਣ-ਅਨੁਕੂਲ", "ਲਾਗੂ ਕਰਨ ਦਾ ਸਭ ਤੋਂ ਵਧੀਆ ਪੜਾਅ"],
    "odia": ["ଚିକିତ୍ସା ବିକଳ୍ପ", "ପ୍ରାଥମିକତା", "ସଫଳତା ହାର", "ସୁସ୍ଥ ହେବାର ସମୟ (ଦିନ)", "ଖର୍ଚ୍ଚ", "ପରିବେଶ ଅନୁକୂଳ", "ପ୍ରୟୋଗର ସର୍ବୋତ୍ତମ ପର୍ଯ୍ୟାୟ"],
}

def _treatment_table_labels(lang: str) -> dict:
    values = _TREATMENT_HEADERS.get(resolve_lang(lang), _TREATMENT_HEADERS["english"])
    return dict(zip(("option", "priority", "success_rate", "recovery_time", "cost", "eco", "stage"), values))
...
.paddy-sidebar-leaves{position:fixed!important;left:0;top:0;width:340px;height:100vh;overflow:hidden;pointer-events:none;z-index:20!important}
.paddy-leaf{position:absolute;top:-30px;width:13px;height:22px;background:#8fca7c;border-radius:100% 0 100% 0;opacity:.84;pointer-events:none;will-change:top,transform;animation:paddy-leaf-fall 11s linear infinite}
.paddy-leaf:after{content:"";position:absolute;left:6px;top:3px;width:1px;height:17px;background:#477a3d;transform:rotate(26deg)}
...
if "🔬 Diagnosis" in page:
    ...
    if uploaded_file:
        ...
        table_labels = _treatment_table_labels(lang)
        treat_rows = [{
